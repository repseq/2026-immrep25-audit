...
SUBSAMPLED = ("vdjdb_hq", "vdjdb_lq")   # the only cohorts too large to lay out in full


def graph_subsets(coh: dict, names: list, chain: str = "B", min_n: int = 30,
                  per_epi: int = 50, seed: int = 42) -> dict:
    """Per-cohort node sets for the homology graph panels.

    Nodes are the CDR3s of every epitope carrying >= min_n records. Every cohort is laid out in
    full except the two VDJdb sets, which sfdp cannot handle (VDJdb HQ carries ~53k TCRbeta nodes
    and does not finish); those are cut to per_epi = 50 receptors per epitope, IMMREP25's own
    depth. Nothing else is touched -- the audited benchmark and its controls are
    already at or below 1000 nodes, so their panels show all the data. An earlier version capped each cohort's
    TOTAL node count at the largest cohort that fitted, which made the cap depend on the chain
    (1000 on TCRbeta, 4211 on TCRalpha) and the two chains' panels incomparable.

    Returns {name: (sub_df, full_n, n_epitopes)}; full_n lets the caller say "N shown of N total".
    """
    out = {}
    for name in names:
        lc = coh[name]["long"]; lc = lc[lc.chain == chain]
        vc = lc.epitope.value_counts(); eps = vc[vc >= min_n].index.to_numpy()
        df = lc[lc.epitope.isin(eps)].reset_index(drop=True)
        if name in SUBSAMPLED:
            sh = df.sample(frac=1.0, random_state=seed)      # shuffle once, then keep the
            sub = sh[sh.groupby("epitope").cumcount() < per_epi].reset_index(drop=True)
        else:
            sub = df
        out[name] = (sub, len(df), int(len(eps)))
    return out
