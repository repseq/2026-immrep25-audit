...
`per_epitope_sn` -- the one-vs-many estimator behind every reported S/N -- drops d = 0
from m and l while keeping every duplicate record in P_self and P_nonself: duplicates
are independent observations, but a sequence is not evidence for itself. This matters
for the VDJdb cohorts, which are 45-77% duplicate CDR3s and, in the HQ case, defined by
repeat observation (>=2 references), so their d = 0 pairs exist by construction. It also
puts the statistic on the same footing as the homology graph, whose edges are Hamming
exactly 1. `count_pairs` / `sn_from_counts` keep the d = 0 term for the publicity view.
"""
...
def per_epitope_sn(long_chain: pd.DataFrame, max_d: int = 3, min_n: int = 30,
    ...
    for e in eps:
        ...
        m_exact = cw["m_exact"].copy(); P_self = cw["P_self"]
        lhist, P_non = _cross_hist_by_len(E.cdr3.to_numpy(), R.cdr3.to_numpy(), max_d)
        lhist = lhist.copy()
        # A sequence is not evidence for itself: identical pairs (d=0) are dropped from the
        # observed counts while every record is kept in the exposures. VDJdb HQ is *defined*
        # as CDR3s supported by >=2 references, so its d=0 pairs exist by construction.
        m_exact[0] = 0.0; lhist[0] = 0.0
        m_cum = np.cumsum(m_exact)
