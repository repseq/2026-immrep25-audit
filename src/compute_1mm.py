...
N_PGEN = 1000     # receptors per cohort and chain (the size of every cohort but IMMREP23).
                  # ponytail: OLGA's per-sequence Hamming-1 pgen is the bottleneck; vdjtools
                  # (~/vcs/code/vdjtools) computes pgen far faster and would let the full 11k
                  # IMMREP23 set be used without sampling, if the distribution ever needs it.
...
if __name__ == "__main__":
    ...
    cohorts = ["immrep25_pos", "immrep22_true", "olga_random", "airr_control", "airr_top"]
    ...
    for name in cohorts:
        for chain in ("A", "B"):
            ...
            pr = coh[name]["paired"]
            # Every cohort here contributes exactly 1000 paired receptors except the seen-peptide
            # IMMREP23 set, which carries 11k; what the figure compares is the shape of the pgen
            # distribution, so the deep cohort is sampled to the same depth as the others rather
            # than run through OLGA eighteen times over. Seeded, so it is reproducible.
            if len(pr) > N_PGEN:
                pr = pr.sample(N_PGEN, random_state=0)
            seqs = pr[col].tolist()
            t = time.time()
