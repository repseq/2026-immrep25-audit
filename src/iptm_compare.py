...
def main():
    ...
    # Inventory of every modelled cohort: epitope, HLA, n complexes, median ipTM. TCRvdb and the
    # VDJdb free pool are loaded here FOR THE TABLE ONLY -- they take no part in the ANOVA, the
    # figure or the macros above, which compare IMMREP25 against its two controls. Their rows are
    # cut exactly as transferability.py cuts them (both scores present, >=15 per class) so the
    # totals agree with the confidence-transfer table.
    TBL = REPO.parent / "2026-immrep25-audit-ms" / "tables" / "iptm_control_table.tex"
    imm_hla = pd.read_csv(REPO / "results" / "immrep25_peptides.csv").set_index("peptide").hla
    m = pe.dataset == "IMMREP25"
    pe.loc[m, "hla"] = pe.loc[m, "epitope"].map(lambda q: "HLA-" + imm_hla[q])

    def _ref(path, fixed_hla=None):
        """Per-epitope inventory for a reference set carrying both classes.

        fixed_hla overrides the metadata's own allele column, for a set whose column is
        unusable: TCRvdb records the null-allele strings A*02:608N and B*08:336N. Both of its
        epitopes are HLA-A*02:01-restricted, checked against the VDJdb 2026-06-03 dump --
        11,891 GLCTLVAML and 1,828 YLQPRTFLL records, every one HLA-A*02 (11,670 and 1,084
        of them written out as A*02:01), with no other allele on any record.
        """
        t = (pd.read_csv(HF / path, sep="\t", low_memory=False)
             .dropna(subset=["y", "epitope", "iptm", "plddt"]))
        ok = [e for e, v in t.groupby("epitope")
              if min((v.y == 1).sum(), (v.y == 0).sum()) >= MIN_N]
        r = (t[t.epitope.isin(ok)].groupby("epitope")
             .agg(size=("iptm", "size"), median=("iptm", "median"), mhc=("mhc", "first"))
             .reset_index())
        r["hla"] = fixed_hla if fixed_hla else r.mhc.map(_short)
        return r

    def _fmt(frame):
        out = []
        for _, r in frame.sort_values("median", ascending=False).iterrows():
            hla = "\\texttt{%s}" % r.hla if r.hla else "--"
            out.append("\\texttt{%s} & %s & %d & %.2f \\\\\n"
                       % (r.epitope, hla, int(r["size"]), r["median"]))
        return "".join(out)

    blocks = [("IMMREP25 unseen peptides, the benchmark under audit", pe[pe.dataset == "IMMREP25"]),
              ("Real VDJdb binders, positive control", pe[pe.dataset == "VDJdb binder"]),
              ("Mismatched TCR--epitope decoys, negative control", pe[pe.dataset == "negatives"]),
              ("TCRvdb, assay-labelled binders and non-binders",
               _ref("tcrvdb/metadata.tsv", "HLA-A*02:01")),
              ("VDJdb free pool, deposited pairings and mispairings",
               _ref("vdjdb_free_pool/metadata.tsv"))]
    body = "\\midrule\n".join("\\multicolumn{4}{l}{\\emph{%s}} \\\\\n%s" % (lab, _fmt(f))
                              for lab, f in blocks)
    # A longtable, not a tabular: at 50 rows this does not fit on one page, and a tabular
    # overruns it silently, dropping the last rows with only an Overfull \vbox in the log. The
    # whole environment is emitted here because \input at the start of an alignment cell makes
    # TeX insert the column template before it sees \multicolumn ("Misplaced \omit"). The
    # caption is the supplement's \structInvCaption, so the prose stays in the manuscript.
    head = ("\\toprule\nEpitope & HLA & $n$ structures & Median ipTM \\\\\n\\midrule\n")
    tbl = ("\\begin{longtable}{llrr}\n"
           "\\caption{\\structInvCaption}\\\\\n"
           + head + "\\endfirsthead\n" + head + "\\endhead\n"
           + body + "\\bottomrule\n\\end{longtable}\n")
    TBL.write_text("% GENERATED by src/iptm_compare.py -- do not edit.\n"
                   ...
                   "% Caption text is \\structInvCaption, defined in the supplement.\n" + tbl)
