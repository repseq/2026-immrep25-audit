...
AXES = ["va", "ja", "vb", "jb"]
AXES_A = ["va", "ja"]        # TCRalpha axes, reported separately in the pairing figure
AXES_B = ["vb", "jb"]        # TCRbeta axes
...
def pairing_per_epitope(paired: pd.DataFrame, min_n: int = 30, n_null: int = 40,
    ...
    for ei, e in enumerate(eps_u):
        ...
        if ne < min_n:
            continue
        # combined over all four axes, and split by chain so the alpha and beta contributions
        # to the gene-usage bias can be read separately
        def _kl_axes(sel, axes):
            return np.mean([_kl(np.bincount(codes[ax][sel], minlength=vocab[ax]), bg[ax]) for ax in axes])
        kl_obs = _kl_axes(idx, AXES)
        kl_obs_a = _kl_axes(idx, AXES_A)
        kl_obs_b = _kl_axes(idx, AXES_B)
        knull = np.empty(n_null); knull_a = np.empty(n_null); knull_b = np.empty(n_null)
        for k in range(n_null):
            s = rng.choice(N, ne, replace=False)
            knull[k] = _kl_axes(s, AXES)
            knull_a[k] = _kl_axes(s, AXES_A)
            knull_b[k] = _kl_axes(s, AXES_B)
        ...
        rows.append({"epitope": e, "n": ne,
                     ...
                     "gene_excess_a": kl_obs_a - knull_a.mean(),
                     "gene_excess_b": kl_obs_b - knull_b.mean(),
