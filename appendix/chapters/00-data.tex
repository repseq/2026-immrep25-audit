...
($\text{cdr3}\alpha,V\alpha,J\alpha,\text{cdr3}\beta,V\beta,J\beta$, epitope).
...
OLGA random  & $P_{\text{gen}}$-matched to immrep25 pos & pure-noise floor \\
\bottomrule
\end{tabular}
\end{center}
...
TCRvdb true/false are dominated by the same two epitopes (YLQPRTFLL, GLCTLVAML),
giving an epitope-matched verified-vs-refuted contrast. Because the number of
epitopes itself inflates S/N, all cross-cohort comparisons use a \emph{matched
design}: $E=2$ epitopes $\times\,K=30$ TCRs, bootstrapped (the two-epitope cap is
imposed by TCRvdb), with permutation nulls making every metric dimensionless.
